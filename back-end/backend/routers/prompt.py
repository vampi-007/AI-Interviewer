...
@router.get("/all-tech-stacks", response_model=List[dict[str, str]])
async def get_all_tech_stacks_endpoint(
    db: AsyncSession = Depends(get_db)
):
    tech_stacks = await get_all_tech_stacks(db)
    
    return [
        {
            "prompt_id": str(stack.prompt_id),  # Convert UUID to string
            "tech_stack": stack.tech_stack,
            "difficulty": stack.difficulty.value  # ✅ Convert Enum to string
        }

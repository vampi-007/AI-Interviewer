...
async def handle_vapi_end_of_call(request: Request, db: AsyncSession):
    try:
        ...
        # ✅ Helper function to convert datetime to naive
        def to_naive_datetime(dt_str):
            if dt_str:
                dt = datetime.fromisoformat(dt_str)
                return dt.replace(tzinfo=None)  # Remove timezone
            return datetime.utcnow()  # Default fallback

        # ✅ Convert timestamps to naive before inserting
        start_time = to_naive_datetime(message.get('startedAt'))
        end_time = to_naive_datetime(message.get('endedAt'))
        ...
        new_interview = Interview(
            ...
            created_at=datetime.utcnow()  # Ensure created_at is also naive
        )

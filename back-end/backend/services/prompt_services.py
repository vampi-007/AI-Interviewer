...
# ✅ Fetch All Tech Stacks with Their Difficulty Levels (No Admin Protection)
async def get_all_tech_stacks(db: AsyncSession) -> List[tuple[UUID, str, Difficulty]]:
    query = select(Prompt.prompt_id, Prompt.tech_stack, Prompt.difficulty).distinct()
    result = await db.execute(query)
    return result.all()
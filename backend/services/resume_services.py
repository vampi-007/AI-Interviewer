...
from backend.config import (
    settings,
)  # Assuming you have a settings module for configurations
...
async def upload_resume(file: UploadFile, db: AsyncSession):
    ...
    try:
        ...
        return JSONResponse(
            content={"error": f"Failed to call LLM: {str(e)}"}, status_code=500
        )
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "Failed to parse response"}, status_code=500
        )
...
async def get_user_resumes(user_id: str, db: AsyncSession):
    ...
    return JSONResponse(
        content={"user_id": str(user_id), "resumes": resume_data}, status_code=200
    )
...
async def get_resume_by_user_id(user_id: str, resume_id: str, db: AsyncSession):
    ...
    result = await db.execute(
        select(Resumes).filter(
            Resumes.user_id == user_id, Resumes.resume_id == resume_id
        )
    )
    ...
    if not resume:
        return JSONResponse(
            content={"error": f"Resume {resume_id} not found"}, status_code=404
        )
...
async def get_resume_by_id(resume_id: str, db: AsyncSession):
    ...
    try:
        result = await db.execute(
            select(Resumes).filter(Resumes.resume_id == resume_id)
        )
        ...
        return JSONResponse(
            content={"error": f"Error retrieving resume: {str(e)}"}, status_code=500
        )
